...
public:
    ...
    //Metodos agregados
    std::vector<std::vector<INodo*>> findConnectedComponents() {
        resetNodes();
        std::vector<std::vector<INodo*>> connectedComponents;
        std::vector<INodo*> currentComponent;

        for (NodoGrafo* node : listaNodos) {
            if (!node->visitado) {
    ...
    void dfs(NodoGrafo* node, std::vector<INodo*>& currentComponent) {
        node->visitado = true;
        currentComponent.push_back(node->getInfo());

        for (Arco* arc : *(node->getArcs())) {
            NodoGrafo* neighbor = static_cast<NodoGrafo*>(arc->getDestino());
            if (!neighbor->visitado) {
